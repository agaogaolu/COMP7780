window.SMOOTH_SCROLLING = 0, gsap.config({
    nullTargetWarn: !1
}), gsap.defaults({
    ease: Quint.easeOut
});
...
const PERFS = {
    ...
    getPerfs = () => {
        ...
        t = deviceSettings.isTablet || deviceSettings.isIE ? PERFS.PERF_BAD : o < 5 ? PERFS.PERF_HIGH : o < 14 ? PERFS.PERF_GOOD : o < 22 ? PERFS.PERF_LOW : PERFS.PERF_BAD, navigator && navigator.connection && "4g" !== navigator.connection.effectiveType && (navigator.connection.effectiveType, t = PERFS.PERF_BAD), PERF = t;
...
const header = getId("header"),
    ...
    SpaceX = () => {
        decodeFeatureImage(), decodeImages(), decodeImagesLoaderResize(), checkDeviceSettings(), addEventListeners(), onResize(), addIO(), animateIn(), gsap.ticker.add(globalRAF), window.addEventListener("resize", onResize), document.fonts && document.fonts.ready.then(onResize)
    },
    addEventListeners = () => {
    ...
    decodeImage = (e, t = (() => {})) => {
        ...
        const n = new Image,
            r = () => {
                e.removeAttribute("data-preload"), e.style.backgroundImage = `url('${n.src}')`, t()
            },
            i = () => {
                n.onload = null, r()
            },
        ...
        n.decode ? (n.src = o, n.decode().then(r).catch(e => s())) : s()
    },
    animateIn = () => {
        if (TIMELINES.globalIn.add(() => getId("wrapper").classList.add("visible"), .1), TIMELINES.globalIn.fromTo(".scrollme", 1.5, {
            ...
            TIMELINES.count.to(e, 1.5, {
                ...
                ease: Expo.easeOut,
                onUpdate: () => {
                    t.innerHTML = Math.round(e.v1), a.innerHTML = Math.round(e.v2), o.innerHTML = Math.round(e.v3)
                }
            }, 0)
        }
    ...
    globalRAF = () => {
        if (SMOOTH_SCROLLING += (cy - SMOOTH_SCROLLING) * EASE, HAS_SMOOTH && scroller ? (scroller.style.transform = `translate3d(0,-${toFixed(SMOOTH_SCROLLING,IS_SCROLLING?3:0)}px,0)`, scroller.style["will-change"] = IS_SCROLLING ? "transform" : "", document.body.style["pointer-events"] = IS_SCROLLING ? "none" : "") : SMOOTH_SCROLLING = cy, PERF >= PERFS.PERF_HIGH && parallax && SMOOTH_SCROLLING <= innerHeight) {
            ...
            if (parallax.style.transform = `translate3d(0,${t}px,0) scale3d(${o}, ${o}, ${o}) rotate3d(0, 0, 0.75, ${a}deg)`, opacity) {
        ...
        const n = new IntersectionObserver(t => {
            ...
            t.forEach((t, n) => {
                ...
                if (i > s.ratio) {
                    ...
                    s.animated = !0, inViewport(t.target, a, d), o++
                }
                s.y = r, s.ratio = i
            })
        }, t);
...
const modal = getId("modal"),
    ...
    openModal = ({
        ...
        }, 0), o.staggerFromTo([modal, modalchild], 1.25, {
    ...
    addXPost = e => {
        ...
        a.id = "player", a.className = "twitter-tweet", a.setAttribute("data-media-max-width", "1080"), a.innerHTML = `\n    <a href="https://twitter.com/SpaceX/status/${e}?ref_src=twsrc%5Etfw"></a>\n  `, t.appendChild(a);
        let o = document.createElement("script");
        o.setAttribute("src", "https://platform.twitter.com/widgets.js"), o.setAttribute("charset", "utf-8"), t.appendChild(o)
    },
    closeModal = () => {
        const e = getId("modal"),
        ...
        if (innerWidth = window.innerWidth, innerHeight = window.innerHeight, showed ? innerHeight > cachedHeight && innerWidth === cachedWidth && (innerHeight = cachedHeight) : (showed = !0, doggyland({
            ...
            queryAll(".slider").forEach(e => {
                ...
                if (o && n && !e.hasAttribute("data-arrow-noresize")) {
                    ...
                    o.style.top = n.style.top = `${i}px`, a && (a.style.position = "absolute", a.style.top = `${i-65}px`, a.style.bottom = "auto")
                }
